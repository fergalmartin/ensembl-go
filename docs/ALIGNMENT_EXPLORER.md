...
When a block selection contains linked rows whose genomes are in the top bar and have local GFF3 data, its header gains the Genome Browser button. The button projects the selected, non-gap alignment cells back to their strand-aware genomic intervals, opens every represented genome at its interval, and leaves the other top-bar genomes inactive. The first implementation opens one region per assembly; if a selection spans more than one region of the same assembly, it opens the region containing the most selected bases and reports that choice.
