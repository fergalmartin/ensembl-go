...
- Generated by [`backend/scripts/build_grch38_slice.py`](../backend/scripts/build_grch38_slice.py)
  ...
  assembly is not in the repository: set `ENSEMBL_GO_LOCAL_DATA` to your installation's
  data directory for working defaults, or pass `--fasta` / `--gff` to point at your own
  copy. With neither, the two flags are required.
