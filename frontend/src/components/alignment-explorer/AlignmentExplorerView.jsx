...
import { emptyWorkspace, createLayer, createFragment, moveSelection, mergeLayers, layerOverlap, tidyLayer, fitCamera, validateLayerWorkspace, constrainCamera, chunkGap, chunkFasta, sourceViewAnchor, workspaceForSave } from './layers'
...
export default function AlignmentExplorerView({theme='dark',config,genomes=[],incoming,onIncomingConsumed}) {
  ...
  const history=useRef({past:[],future:[]}),stateRef=useRef(state),canvas=useRef(null),file=useRef(null),metadata=useRef(null),workspace=useRef(null),epoch=useRef(0),incomingHandled=useRef(null),genomesRef=useRef(genomes)
  ...
  const {tiles,annotations,connections,counts,pending,warnings,displayCamera}=useLayerData(dataset,layer,renderState.camera,size,state.annotations,revision,setError)
  ...
  useEffect(()=>{
    ...
    let cancelled=false
    api(`/datasets/${dataset.id}/blocks/${state.sourceBlock}/rows`).then(value=>{if(!cancelled)setSource(value)}).catch(e=>{if(!cancelled)setError(e.message)})
    return()=>{cancelled=true}
  },[dataset,source?.block,state.sourceBlock])
  async function sourceBlock(id){
    try{const result=await api(`/datasets/${dataset.id}/blocks/${id}/rows`);setSource(result);patch({sourceBlock:Number(id),original:true,selection:[],camera:fitCamera({fragments:[createFragment(result.block,0,result.length,result.rows.map(r=>r.id),{x:result.layout_start||0})]},size.width,size.height)})}catch(e){setError(e.message)}
  }
  ...
  const visibleSourceBlock=sourceViewAnchor(originalFragments,renderState.camera)?.sourceBlock||state.sourceBlock
  const selectedFragment=active.fragments.find(f=>f.id===range.fragment)||active.fragments[0]
  return <section className={`alignment-layers ${theme==='light'?'light':''} ${selectionDrag?'is-selection-dragging':''}`} ref={explorerRoot} aria-label="Alignment Explorer" data-screenshot-capture="view">
    ...
    {!dataset?<div className="al-welcome"><div className="al-mark">▱<br/>▱<br/>▱</div><h2>Follow sequences across layers</h2><p>Start with a familiar alignment. Select a region, move it to a layer, and connect the pieces that matter.</p><div><button className="primary" disabled={busy||!!job} onClick={()=>setDialog('open')}>Open an alignment</button><button disabled={busy||!!job} onClick={()=>startImport({name:'Layer exploration example',format:'fasta',content:demoAlignment()})}>Explore example</button></div><small>MAF · aligned FASTA · XMFA · Stockholm · Clustal · PHYLIP<br/>All sequences are included, with or without linked genomes.</small></div>:<div className="al-workspace">
      <aside className={`al-sidebar ${state.sidebarCollapsed&&!selectionDrag?'collapsed':''}`}>
        ...
        {!!selectionRows&&<button className={`al-new-layer-drop ${selectionDrag?.target==='new'?'selection-drop-hover':''}`} data-selection-drop="new" onClick={()=>transfer(false,'new')}><b>＋</b><span>New layer<small>Drop the highlighted region here</small></span></button>}
        </div>
</details>
        <details className="al-section" open={selectionRows>0?true:undefined}><summary>Selection</summary><p className="al-hint">Select once, then drag the highlighted region to a layer or ＋ New layer.</p><button onClick={()=>{setRange({fragment:active.fragments[0]?.id||'',start:(active.fragments[0]?.start||0)+1,end:Math.min(active.fragments[0]?.end||100,(active.fragments[0]?.start||0)+100)});setSelectRows([]);setDialog('select')}}>Select by coordinates</button>        {!!selectionRows&&<div className="al-selection-bar" role="region" aria-label="Selected region actions"><strong>{selectionRows?`${selectionRows} selected sequences`:'Select a region'}</strong><small>{selectionRows?`${state.selection.length} rectangles · ${selectionCells.toLocaleString()} cells`:'Choose Rectangle or Columns, then drag across the panel.'}</small>{!!selectionRows&&<><select aria-label="Move selection to layer" value={target} onChange={e=>setTarget(e.target.value)}><option value="new">New layer…</option>{state.layers.filter(l=>l.id!==state.active||state.original).map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select>{target==='new'&&<input aria-label="New layer name" placeholder={`Layer ${state.layers.length+1}`} value={layerName} onChange={e=>setLayerName(e.target.value)}/>}<button className="primary" onClick={()=>transfer(false)}>{state.original?'Place in layer':'Move to layer'}</button>{!state.original&&<button onClick={()=>transfer(true)}>Copy instead</button>}<button onClick={()=>patch({selection:[]})}>Clear selection</button></>}</div>}
</details>
    ...
    {dialog&&<div className="al-modal-shade"><div className="al-modal" role="dialog" aria-modal="true" aria-label={typeof dialog==='string'?dialog:'Rename layer'}><button className="al-close" aria-label="Close dialog" onClick={()=>setDialog(null)}>×</button>
      ...
      {dialog==='select'&&<><h3>Select by alignment coordinates</h3><label>{state.original?'Source block':'Chunk'}<select value={selectedFragment?.id||''} onChange={e=>{const f=active.fragments.find(f=>f.id===e.target.value);setRange({fragment:f.id,start:f.start+1,end:f.end});setSelectRows([])}}>{active.fragments.map(f=><option key={f.id} value={f.id}>Block {f.sourceBlock} · {f.start+1}–{f.end}</option>)}</select></label><div className="al-range"><label>First column<input type="number" min={(selectedFragment?.start||0)+1} value={range.start} onChange={e=>setRange({...range,start:Number(e.target.value)})}/></label><label>Last column<input type="number" value={range.end} onChange={e=>setRange({...range,end:Number(e.target.value)})}/></label></div><label>Sequences <small>None checked means all rows in this region.</small><input placeholder="Search sequences" value={rowQuery} onChange={e=>setRowQuery(e.target.value)}/></label><div className="al-row-choices">{inventory.filter(r=>selectedFragment?.rowIds.includes(r.id)&&(r.label||r.source).toLowerCase().includes(rowQuery.toLowerCase())).slice(0,200).map(r=><label key={r.id}><input type="checkbox" checked={selectRows.includes(r.id)} onChange={e=>setSelectRows(v=>e.target.checked?[...v,r.id]:v.filter(id=>id!==r.id))}/>{r.label||r.source}</label>)}</div><button className="primary" disabled={!selectedFragment||range.end<range.start||range.start<=selectedFragment.start||range.end>selectedFragment.end} onClick={()=>{patch({mode:'pan',selection:[{fragmentId:selectedFragment.id,start:range.start-1,end:range.end,rowIds:selectRows.length?selectRows:selectedFragment.rowIds}]});setDialog(null)}}>Select region</button></>}
